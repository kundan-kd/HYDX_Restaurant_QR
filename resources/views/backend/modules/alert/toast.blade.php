<!----------------Status of room types,category.. changed success alert------------->
<div class="toast-container position-fixed top-3 end-0 p-3 toast-index toast-rtl">
    <div class="toast hide" id="liveToast" role="alert" aria-live="assertive" aria-atomic="true">
        <div class="toast-body bg-success text-white alert_msg">
            Successfully
        </div>
    </div>
</div>

<!----------------Status of room types,category.. changed success alert------------->
<div class="toast-container position-fixed top-3 end-0 p-3 toast-index toast-rtl">
    <div class="toast hide" id="liveToast2" role="alert" aria-live="assertive" aria-atomic="true">
        <div class="toast-body bg-danger text-white alert_msg_danger">
            Successfully
        </div>
    </div>
</div>

{{-- ...................................................................................... --}}
  <!----------------Toast ALert display on ajax call action using function call (toastSuccessAlert,toastWarningAlert,toastErrorAlert)in js files------------->
<div class="toast-container position-fixed top-3 end-0 p-3 toast-index toast-rtl">
    <div class="toast hide" id="liveToastSuccessAlert" role="alert" aria-live="assertive" aria-atomic="true">
        <div class="toast-body bg-success text-white toast-alert-success-msg">
            Successfully
        </div>
    </div>
</div>

<div class="toast-container position-fixed top-3 end-0 p-3 toast-index toast-rtl">
    <div class="toast hide" id="liveToastWarningAlert" role="alert" aria-live="assertive" aria-atomic="true">
        <div class="toast-body bg-warning text-white toast-alert-warning-msg">
            Warning
        </div>
    </div>
</div>

<div class="toast-container position-fixed top-3 end-0 p-3 toast-index toast-rtl">
    <div class="toast hide" id="liveToastErrorAlert" role="alert" aria-live="assertive" aria-atomic="true">
        <div class="toast-body bg-danger text-white toast-alert-error-msg">
            Error
        </div>
    </div>
</div>

{{-- ------------------------------------------------------------ --}}